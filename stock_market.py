...
def build_tree(price, shortMA, longMA):
    ...
    root.left = TreeNode(values[0][0], values[0][1])
    root.right = TreeNode(values[1][0], values[1][1])
...
if __name__ == "__main__":
    ...
    print("Binary Tree:")
    print(f"   {tree.label}:{tree.value}")
    print("  /           \\")
    print(f"{tree.left.label}:{tree.left.value}   {tree.right.label}:{tree.right.value}")
